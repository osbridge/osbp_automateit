...
cronedit(default_user) do
  ...
  delete "*/1 * * * * if test -f /var/www/bridgepdx_redmine/current/Rakefile; then (cd /var/www/bridgepdx_redmine/current && rake redmine:my:getemails --silent RAILS_ENV=production); fi"
  append "*/1 * * * * if test -f /var/www/bridgepdx_redmine/current/Rakefile; then (cd /var/www/bridgepdx_redmine/current && nice -n19 rake redmine:my:getemails --silent RAILS_ENV=production); fi"
